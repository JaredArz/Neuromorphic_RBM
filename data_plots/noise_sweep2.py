    ...
    for i in range(len(mdd)):
        for j in range(len(scale)):
            for k in range(len(gdd)):
                ...
                means,stddevs,highs,lows = map(to_float, get_file_data(root_dir,data_file,glob))
                all_data[i,j,k] = data(means,stddevs,highs,lows)

    plot_init()
    plot_sweep_slice(gdd,all_data[0,0,:])
    plt.savefig(f"./help.png")
...
def get_file_data(root_dir, data_file,const_params=None):
...
def select_dir(root_dir,c):
    if c is not None:
        print(f'{root_dir}/*Gdd{c["gdd"]}*_*Gcc{c["gcc"]}*_*Ndd{c["mdd"]}*_*s{c["s"]:.2e}*')
        dir_path = glob.glob(f'{root_dir}/*Gdd{c["gdd"]}*_*Gcc{c["gcc"]}*_*Ndd{c["mdd"]}*_*s{c["s"]:.2e}*')
    else:
